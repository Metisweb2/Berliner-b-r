    ...
    <Section id="contact">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center">
        ...
        <div className="relative h-[400px] lg:h-[600px] rounded-3xl overflow-hidden border border-white/10 group bg-[#0a0a0a]">
          ...
          {/* Custom Dark Mode Filter for standard Google Maps iframe */}
          <iframe 
          ...
          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 pointer-events-auto flex justify-center w-full px-4">
            <a
              href="https://www.google.com/maps/dir/?api=1&destination=Marzahner+Promenade+25,+12679+Berlin"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-3 bg-[#D90429] hover:bg-[#b00320] text-white font-bold py-3.5 px-8 rounded-full shadow-[0_10px_30px_rgba(217,4,41,0.5)] transition-all hover:scale-105 active:scale-95 whitespace-nowrap border border-white/10 backdrop-blur-md"
            >
              <Navigation className="w-5 h-5 fill-white/20" />
              <span>Route zur Fahrschule</span>
            </a>
          </div>
        </div>
      </div>
...
console.log('Successfully updated Location.tsx with simple iframe map');
