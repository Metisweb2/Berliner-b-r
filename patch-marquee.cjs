      ...
      <div className="max-w-[1600px] mx-auto px-4 md:px-8 mb-16 text-center">
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8 text-white/90">
          <p className="text-sm md:text-base font-bold tracking-widest uppercase">
            Oder brauchst du noch mehr Impressionen?
          </p>
          <a href="#" className="text-xs md:text-sm font-semibold hover:text-white transition-colors uppercase tracking-wider underline underline-offset-4 text-primary">
            Alle Posts auf TikTok &rarr;
          </a>
        </div>
      </div>
...
function VideoCard({ video, onClick }: { video: VideoData, onClick: () => void }) {
  return (
    <div
      className="relative flex-none w-[240px] md:w-[280px] aspect-[9/16] rounded-2xl overflow-hidden cursor-pointer group shadow-2xl bg-neutral-900 transition-transform duration-500 hover:scale-[1.02]"
      onClick={onClick}
    >
      <img 
